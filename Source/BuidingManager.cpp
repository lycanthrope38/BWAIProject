	...
	while (!builder->build(building, buildPosition))
	{
		...
		do{
			...
			if (closestCentre != NULL)
			{
				...
				if ((3 < buildPosition.getDistance(closestCentre->getTilePosition())) && (buildPosition.getDistance(closestCentre->getTilePosition()) < MINERALDIST))
				{
